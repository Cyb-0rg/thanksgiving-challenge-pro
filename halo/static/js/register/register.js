...
wholeForm.addEventListener("click", event => {
    ...
    if (pageInfo.pageValidationStatus && pageInfo.individualValidation.get(pageInfo.currentPage)) {
        ...
        pageInfo.currentPage += increment;
        showCurrentPage();
        //this else if is added to the original halo code to make it possible to use prev button
    } else {
        generalErrorText.classList.add("d-none") 
        //remove changed to add to just prevent showing id="error-reminder-container" on register.html
        //needs to be fixed later. 
        // currently id="error-reminder-container" is totally hidden
    }
})

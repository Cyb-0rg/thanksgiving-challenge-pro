...
def minor_waiver_form_submission(hacker, link):
    subject = "{}, Christmas Challenge Minor Waiver Form".format(hacker.first_name)
    from_email=FROM_EMAIL
    to_email=[hacker.email]
    body = '''
        Hey {},

        Thanks again for registering for the Christmas Challenge. We require all minors (under 18), to bring a signed waiver form by your primary care taker.
...
def new_waitlister_added(email, name):
    subject = "Christmas Challenge Waiting List Success! Registration Opens Soon!"
    from_email=FROM_EMAIL
    to_email = [email]
    body = '''
    Hey {}, 
        We are totally pumped! Christmas Challenge is coming soon! We can't wait for you to join us in celebrating another awesome year of hacking at The College of New Jersey!
